...
export default async function Home() {
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      ...
      {/* 🔥 新增：兄弟社区展示区域 - 移动端友好 */}
      <section id="communities" className="relative py-20 overflow-hidden premium-gradient">
        <div className="absolute inset-0">
        ...
        <div className="section-shell relative z-10 space-y-10">
          <div className="text-center space-y-4">
            <div className="inline-flex items-center gap-3 px-4 py-2 rounded-full glass-card neon-border">
              ...
              <span className="text-sm font-medium">{c.communities?.tag || "兄弟社区"}</span>
            </div>
            <h2 className="text-4xl md:text-5xl font-black premium-text-gradient">
              {c.communities?.title || "携手共建 AI 生态"}
            </h2>
            <p className="text-foreground/70 max-w-2xl mx-auto">
              {c.communities?.description || "与优秀社区深度合作，共享资源，互通有无，打造 AI 领域最具活力的社区联盟"}
            </p>
          </div>
          ...
          {data.communities.length > 0 ? (
            <div className="relative overflow-hidden py-8">
              ...
              {/* 滚动容器 */}
              <div className="flex gap-6 animate-scroll">
                {[...data.communities, ...data.communities].map((community: any, index: number) => (
                  <a
                    key={`${community.id}-${index}`}
                    href={community.website || community.url || "#"}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex-shrink-0 w-64 glass-card rounded-2xl p-6 hover:scale-105 transition-all group"
                  >
                    {/* Logo 展示 */}
                    {community.logo && (
                      <div className="w-full h-32 rounded-xl bg-white/90 dark:bg-white/80 p-4 flex items-center justify-center border border-black/5 dark:border-white/10 shadow-sm mb-4">
                        <img
                          src={community.logo}
                          alt={community.name}
            ...
            <div className="text-center py-12 text-foreground/60">
              ...
              <p>暂无兄弟社区信息</p>
              <p className="text-sm mt-2">请在后台"合作伙伴"中添加类型为"兄弟社区"的伙伴</p>
            </div>
          )}
        </div>
        <div className="grid-overlay" />
