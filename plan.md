...
- [ ] E1 `content/interview/index.json` 增加 `category` 字段；存量 17 篇补齐分类（映射表见 §3.1）
- [ ] E2 新路由 `/interview/category/[cat]`：分类落地页（简介 + 文章列表 + 真题集置顶）
- [ ] E3 新组件「额外链接卡」（站内笔记 / 官方文档），样式沿用 `.ivu-paper` 顶条换紫
- [ ] E4 详情页右栏新增「同分类文章」列表（当前分类、按 no 序，排除自身前 6 篇）
- [ ] E5 `/interview` 列表页改版：8 分类卡置顶（图 + 篇数 + 一句话），最新更新列表下移
- [ ] E6 子站顶栏加分类快捷 chips
- [ ] E7 frontmatter 与列表/详情显示 `updated` 字段（最后更新时间）
- [ ] E8 移动端分类页适配（分类卡单列、chips 44px）
- [ ] E9 `lib/interview.ts` 支持 category 过滤与分类元数据（名称/简介/篇数）
- [ ] E10 面包屑升级：首页 / 面试间 / 分类 / 文章
