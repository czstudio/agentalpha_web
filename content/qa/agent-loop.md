...
干预手段按烈度分级。轻的：往上下文里注入一段干预提示——「你已经连续三次调用同一工具且失败，请换一种方法或总结当前障碍」；中的：强制切换路线，比如从「继续重试」切到「重新规划」；重的：终止任务，把已收集的信息打包交还人工。核心思想就一句：永远不要让模型自己决定「要不要停下来」，停不下来是它的本性，停下来必须是外部的机制。
