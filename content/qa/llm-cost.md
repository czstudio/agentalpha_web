...
第一层：省 token。这是性价比最高的一层。上下文瘦身——提示词里冗长的说明、few-shot 例子、塞进去却没用的检索内容，都是按 token 计费的死重，逐项审计砍一半很常见。输出限制——回答长度、列表条数、格式化输出，输出 token 通常比输入贵，控制输出就是直接省钱。工具结果别全量进上下文，摘要进。
...
落地的度量基础：先埋成本指标——每次调用的 token 数按「输入/输出/模型档位」拆开记录，按天看曲线。没有成本归因，优化就是拍脑袋。
