...
数据源不同：RAG 检索的是外部文档——产品手册、制度文件，内容是别人写的、相对静态、有版本管理。记忆检索的是这个用户和这个 Agent 的历史交互，内容是系统自己生成的、碎、口语化、没有现成的版本管理。
...
一句话收束：RAG 是「查图书馆」，记忆是「翻自己的笔记本」——笔记本会记错、会过时、还可能被人塞假条子，所以要多一套管理机制。
