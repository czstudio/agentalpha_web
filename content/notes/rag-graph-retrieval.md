...
title: "Graph RAG 不是给知识库画一张图：关键是让证据沿关系可回溯"
excerpt: "当问题需要跨文档、跨实体、多跳关系时，图结构能补足纯向量相似度的短板。设计时要把节点、边、来源、时间和路径评分一起纳入，而不是只展示一个漂亮的关系网络。"
series: "RAG"
seriesNo: "03"
number: "67"
minutes: 23
...
“这家公司在 2024 年收购的团队后来用了哪个模型？”不是找一段最相似的文字就能稳答的问题。它需要沿着公司—事件—团队—时间—模型几层关系走，并且每一条边都能回到原文。Graph RAG 的难点不是建图，而是让图上的路径成为可审计证据。

## 先给一个能复述的答案

Graph RAG 适合实体关系明确、问题需要多跳推理或跨文档对齐的场景。先从文档抽取带来源的节点和边，做实体消歧、时间和权限校验，再用向量召回找到入口节点，沿有限步数扩展候选路径，最后把路径上的原文片段交给生成器。图不是答案，路径和原文引用才是答案的证据。
