...
title: "多模态模型不是给图片加一个输入框：图像如何进入 Transformer"
excerpt: "图片要被语言模型理解，必须先经过视觉编码、特征投影和跨模态融合。把像素、patch、视觉 token 和文本 token 串起来，才能解释模型到底看到了什么。"
series: "多模态"
seriesNo: "08"
number: "34"
minutes: 19
...
做一个“看发票并生成报销摘要”的 Agent 时，最容易犯的错误是把图片当成一段很长的字符串塞进 prompt。结果模型能认出公司名，却把金额、日期和表格列对应错。问题不在于它不会中文，而在于图片还没有被转换成适合 Transformer 处理的表示。
...
一个 patch 可能只包含半个数字、表格边框或人物轮廓。视觉编码器通过多层注意力，让局部 token 逐渐获得更大的感受野。因此，不能把每个视觉 token 简单解释成一个完整物体，也不能用 token 数量直接等价于图片里有多少信息。
