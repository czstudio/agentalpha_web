...
title: "长上下文不是把窗口拉长：先解决信息进入、留存和被正确引用"
...
把上下文窗口从 32k 配成 128k，产品页上的数字变大了，用户却可能仍然找不到藏在中间的那句规定。长上下文有三层问题：信息是否被送进窗口，模型是否在长距离中保留它，最终答案是否引用了正确证据。只讨论最大 token 数，会把三层问题混成一个宣传指标。
...
预算不能只留给输入。若输出要生成结构化报告、引用和下一步动作，就必须提前保留输出空间；工具结果也要限制每个调用的最大 token。
