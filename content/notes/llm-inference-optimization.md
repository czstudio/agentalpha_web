...
title: 大模型推理优化不是只换量化：吞吐、延迟和显存怎么一起看
excerpt: 推理优化没有一个“万能开关”。先把 TTFT、TPOT、吞吐和显存分开，再选择量化、连续批处理、Prefix Cache、投机解码与并行策略，才能知道优化到底改善了什么。
series: "LLM 基础"
seriesNo: "04"
number: "18"
minutes: 20
...
“把模型从 FP16 换成 INT4，速度就会快四倍吗？”

如果你在面试里直接回答“会”，大概率会被继续问：为什么首 token 延迟没降？为什么 batch 一大反而变慢？为什么显存省了，P99 却变差？

推理优化最容易犯的错误，是把所有问题都归到“模型太大”。实际服务里，慢可能来自 prompt 太长、KV Cache 读取、GPU 利用率低、请求长度不齐、跨卡通信、CPU 调度或尾延迟控制。优化前先建立指标地图，才能避免换了一种量化，结果只是把瓶颈挪到另一处。
...
比较两个版本时，必须写清口径和数据分布。一个版本可能 output tokens/s 提升了，但因为输出更短，requests/s 并没有改善。
