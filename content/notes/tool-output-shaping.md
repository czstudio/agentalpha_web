...
excerpt: "工具回得越多，Agent 不一定看得越明白。先定观察对象和字段优先级，再处理分页、截断、引用，下一步才不会被一堆无关 JSON 带跑。"
series: "工具调用"
seriesNo: "10"
number: "47"
minutes: 24
...
很多团队遇到这种问题，第一反应是换更大的模型。更常见的原因是：工具把数据库响应原封不动地倒进上下文，却没有告诉 Agent 哪些字段重要、结果是否完整、下一页怎么取。

## 拆完先给结论

工具输出要为下一步决策设计，而不是照着数据库结构返回。先定义观察契约：摘要、关键字段、完整性标记、分页游标、来源和错误；再按任务投影字段、限制大小、保留证据引用。大结果用分页或二次查询，不要静默截断；任何“没有找到”都必须说明搜索范围和是否完整。输出越短不一定越好，关键是让 Agent 知道它看到了什么、没看到什么，以及下一步能怎么查。
