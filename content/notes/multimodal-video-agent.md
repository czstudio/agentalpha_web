...
title: "视频 Agent 如何处理百万级帧？采样、时间定位和证据回放"
excerpt: "视频 Agent 不会把每一帧都塞给模型。它要先建立时间轴，用镜头切分、关键帧、语音和 OCR 做粗到细检索，再把答案落回可回放的时间区间。"
series: "多模态"
seriesNo: "08"
number: "38"
minutes: 21
...
监控视频一天有 86400 秒，会议录像有几十个小时。用户问“设备第一次冒烟前，谁按了红色按钮”，如果把所有帧逐张送给视觉模型，成本和延迟都不可接受；如果只抽每分钟一帧，又可能正好错过两秒钟的关键动作。

## 先给一个能复述的答案

视频 Agent 应先建立可检索的时间轴，而不是直接做逐帧问答。第一层用低成本信号做粗筛：镜头边界、音频转写、OCR、目标检测和低频关键帧；第二层根据问题定位候选时间窗，再提高帧率或截取短片段做精读。每个证据要记录 `video_id`、起止时间、帧号、来源流和抽取方法，最终回答要能跳回原视频。百万级帧的核心是分层采样、时间定位、上下文预算和证据回放，而不是单纯堆更大的视频模型。
...
这些信号要共享同一时间基准，否则“字幕时间”和“视频时间”对不上，后面的证据回放会漂移。
