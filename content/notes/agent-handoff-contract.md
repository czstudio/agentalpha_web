...
excerpt: "多智能体系统最容易被忽略的不是角色数量，而是交接边界。把任务、状态、证据、权限、幂等键和确认协议写成合同，协作才不会变成一群 Agent 互相甩锅。"
series: "多智能体"
seriesNo: "07"
number: "64"
minutes: 23
...
“让一个 Agent 把任务交给另一个 Agent”听起来像一次消息转发，真正上线后却更像一次跨服务事务：接收方可能不认识发送方，任务可能已经执行过，附件可能过期，回复可能丢在网络里。没有合同的 handoff，只是把不确定性从一个循环搬到了另一个循环。
...
如果这四类都叫 `handoff`，日志里就看不出“建议没采纳”和“任务已经换了负责人”的区别。建议在协议层明确 `mode`，在指标层分别计算接受率、提交率和升级率。
