...
title: "LLM-as-a-Judge 为什么会偏？从评分 rubric 到校准"
excerpt: "让一个大模型给另一个模型打分很方便，但它会偏爱更长、更像自己、更有格式感的答案。可靠的 Judge 需要 rubric、盲评、程序规则和人工样本共同校准。"
series: "评测"
seriesNo: "09"
number: "42"
minutes: 26
...
团队把两个 Agent 的回答交给同一个评测模型，结果 A 几乎总是赢。后来人工抽查发现，A 只是写得更长，喜欢把结论重复三遍；B 的答案更短，却更准确。再把答案顺序互换，胜负也跟着换了。

LLM-as-a-Judge 是有用的放大器，不是天然公正的裁判。它会受到位置、长度、文风、品牌名、提示词和上下文顺序影响。要把它用在面试题、RAG 或 Agent 评测里，必须先定义可观察的 rubric，再用盲评、成对比较、程序断言和人工标注校准。
...
越是客观、可执行的指标，越不要交给 Judge 猜。把“订单是否改成功”写成 API 状态断言，比让模型判断“看起来完成了”可靠得多。

## 二、Rubric 要写成观察项，不要写形容词

“回答质量高”不能直接评分。好的 rubric 会描述行为和证据：
