...
title: "从需求到补丁：一个可靠 Code Agent 的任务分解、修改和验收"
excerpt: "可靠的 Code Agent 不把一句需求直接变成一大段代码，而是先拆成可验证的任务，再沿仓库证据做小步修改，最后用分层验收证明补丁真的能交付。"
series: "Code Agent"
seriesNo: "02"
number: "49"
minutes: 27
...
“把这个接口加上批量导出，顺便修一下权限问题。”这是产品经理眼里的一句话，却不是 Code Agent 可以直接执行的一个动作。

如果 Agent 立刻打开 `export.ts` 开始写，通常会遇到三个问题：它不知道“批量”是同步还是异步，不知道权限来自中间件还是业务层，也不知道验收要看文件下载、任务状态还是审计记录。最后的补丁可能能编译，但人不敢合并。
...
未知项不能被模型偷偷补成“合理默认值”。如果不能马上问人，就把假设写进计划，并为假设配一条验证动作。
