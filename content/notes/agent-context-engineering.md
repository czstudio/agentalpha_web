...
一个客服 Agent 开始很聪明，跑到第十轮后却反复引用已经废止的政策。工程师第一反应是换更大的模型，后来把 trace 展开才发现：完整聊天记录、三次失败工具返回和两份旧版文档都被原样塞进了上下文。
...
上下文窗口不是仓库。每一次模型调用都应该问一句：**如果删掉这段信息，当前决策会变差吗？**如果答案是否定的，它就不应该占据主窗口。
