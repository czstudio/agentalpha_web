...
excerpt: "学习 Agent 不是把论文、框架和面试题各自刷完。用基础知识解释项目，用项目暴露问题，再用论文和面试复述验证，四条线循环推进，才不会学完仍然不会做。"
series: "通用与软实力"
seriesNo: "12"
number: "59"
minutes: 25
...
很多人把学习路线排成一张长清单：Transformer、RAG、LangChain、论文、面试题，一个都不能少。结果是每个词都见过，遇到真实需求却不知道先问什么、怎么验证、出了问题看哪条日志。

## 先给一个能复述的答案

Agent 学习应该用四条线循环：基础负责解释机制，项目负责制造真实约束，论文负责提供更好的假设，面试负责检查能否在有限时间复述。每一轮只选一个可交付问题，把“读懂”变成代码、实验、故障复盘或一段 60 秒回答，而不是不断增加收藏夹。
...
每四周清理一次路线：删掉没有服务于当前目标的收藏，保留能解释项目决策的资料。
