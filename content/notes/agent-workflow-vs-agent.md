...
title: "Workflow 还是 Agent：先看不确定性，再决定要不要把方向盘交出去"
excerpt: "面试里的 Workflow vs Agent 不是概念二选一，而是一次边界设计。把不确定性、风险、验证成本和回退方式放在同一张表里，答案才不会停在‘Agent 更灵活’。"
series: "Agent 架构"
seriesNo: "06"
number: "61"
minutes: 22
...
产品经理说：“这段流程要更智能，用户一句话就自动完成。”工程师很容易顺着需求把整个流程改成 Agent。两周后，系统开始在低风险任务上绕路，在高风险任务上乱试，大家又回头问：是不是本来用 Workflow 就够了？
...
大多数业务只在第二类不确定性上需要 Agent。把输入、结果和目标都交给自由循环，等于把最重要的约束变成了概率问题。
