...
title: "一次 Agent 实验怎样算可复现？从版本指纹到结果归因"
excerpt: "同一套 Prompt 第二次跑出不同结果，不一定是模型随机；工具、知识库、系统时间、并发和评测器都可能变了。可复现实验需要锁住完整的运行环境，并记录能解释结果的证据。"
series: "评测"
seriesNo: "09"
number: "43"
minutes: 21
...
Agent 实验的可复现，不是要求每个 token 都完全一样，而是要能在约定的误差范围内复现结论，并知道偏差来自哪里。没有完整版本指纹，所谓 A/B 结果很可能只是环境漂移。
...
LLM 采样开启时，逐 token 一致并不现实。更重要的是：硬约束不被破坏、任务成功率的差异在置信区间之外、失败类型没有悄悄变坏。
