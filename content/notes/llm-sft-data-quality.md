...
title: "SFT 不是把答案背下来：一条训练样本如何改变模型行为"
excerpt: "监督微调真正改变的是模型在特定输入下的行为分布。把数据构造、loss mask、质量分层和回归评测拆开，才能知道模型学会了什么。"
series: "LLM 训练"
seriesNo: "05"
number: "19"
minutes: 18
...
这几问说明，SFT 不是把答案灌进参数里，也不是简单地让训练集上的 token 概率变高。它是在一批带条件的示例上，重新塑造模型“遇到什么输入、应该采取什么输出行为”的概率分布。

这篇文章只回答一个核心问题：**一条 SFT 样本从数据进入训练，再到最终验收，中间到底发生了什么？**
...
如果把整段对话每个 token 都设成 1，会发生什么？模型也会被要求复现用户问题、角色标签和模板固定前缀。数据里一旦有明显格式偏置，loss 可能下降得很漂亮，真实任务却不一定改善。
