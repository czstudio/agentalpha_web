...
可以把这些字段看成“向量的身份证”。没有身份证的向量，分数再高也不应该直接进入生产上下文。
...
| 版本 | 自然语言 Recall@10 | 错误码 Recall@10 | 证据完整率 | P95 | 单千次成本 |
| --- | ---: | ---: | ---: | ---: | ---: |
...
表中的 `measured` 必须替换成真实回放数据，不能为了文章好看编数字。更重要的是，选择依据应同时包含质量、延迟、显存/存储和迁移成本；平均分略高但索引体积翻倍，未必是更好的生产方案。

## 十五、分层面试题：把“向量”讲成工程决策
