...
title: "Agent 到 Agent 的通信边界：先设计 envelope，再讨论协议名字"
excerpt: "A2A 场景最先要解决的不是选哪个协议，而是消息如何寻址、鉴权、版本化、幂等和回执。把传输层和任务层分开，才能在不同 Agent、队列和网关之间迁移。"
series: "工具调用"
seriesNo: "10"
number: "69"
minutes: 21
...
两个 Agent 互相调用时，最容易把“可以发消息”误认为“可以可靠协作”。真正的系统需要回答：我在调用谁？对方能做什么？这条消息是否过期？如果网络重试会不会重复副作用？回复是建议还是承诺？这些问题属于通信边界，不应该散落在提示词里。
...
大 payload 放引用，避免把整段文档复制到每个 Agent；但引用必须带租户、版本和访问控制。只把 `payload_ref` 当成公开 URL，会把权限问题推迟到最危险的地方。
