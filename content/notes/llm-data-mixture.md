...
title: "预训练数据不是越多越好：清洗、去重和数据配比如何影响能力"
excerpt: "大模型吃下的 token 越多，不代表学到的能力越多。重复网页会放大偏见，模板化代码会挤占推理数据，低质量语料会让训练预算变成噪声。把去重、质量分层、数据混合和能力评测连起来，才能回答‘这批数据值不值得加进来’。"
series: "LLM 训练"
seriesNo: "05"
number: "23"
minutes: 18
...
预训练数据工程的核心不是“收集更多”，而是把有限的训练预算花在真正有信息增益的 token 上。数据清洗、精确去重、近重复去重、质量打分和混合配比共同决定模型会学到什么；任何一个环节粗糙，都可能把规模变成噪声放大器。
...
每层都应该保留 ID 和版本。否则发现模型背错某个事实时，你只能重新下载一遍数据，无法定位是哪个来源、哪次清洗或哪条配比引入的。
