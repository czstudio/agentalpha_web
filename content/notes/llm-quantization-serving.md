...
title: "量化不是把模型变小就结束：质量、带宽和服务形态要一起看"
...
线上模型服务里，显存和带宽常常比 FLOPs 更早成为瓶颈。把 FP16 权重换成 INT8 或更低精度，可能显著降低内存和传输成本，但也可能让少数关键层、长输出或结构化生成变得不稳定。量化是服务系统的一部分，不是一个孤立的压缩脚本。
...
其中 $b$ 是 bit 数，$s$ 是尺度。数值范围内的异常值会拉大尺度，让大多数值的有效分辨率变低；因此常见优化是 per-channel、group-wise 或异常值分离，而不是所有权重共用一个尺度。
