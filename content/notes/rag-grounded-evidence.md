...
title: "RAG 答案看着对，怎么证明它真的有依据？"
excerpt: "引用一个文档链接不等于有依据。要验证 RAG 的答案，必须把结论拆成可核查的 claim，再检查证据是否覆盖、是否矛盾、是否来自正确版本。"
series: "评测"
seriesNo: "09"
number: "40"
minutes: 19
...
产品经理问：“这个季度的退款率是多少？”RAG 返回了一段很流畅的总结，还附了三个链接。打开链接后才发现：第一个是上季度报告，第二个只支持订单量，第三个来自测试环境。

答案看起来对，和答案有依据，是两件事。RAG 的引用评测不能只检查“有没有链接”，而要检查每一个可验证的结论，是否由正确、足够、可定位的证据支持。
...
“本季度退款率下降了 2 个百分点，主要因为风控策略上线。”至少包含两个 claim：一个是数值变化，一个是因果解释。它们可能需要不同证据，不能让一条链接包打天下。
...
- [LLM-as-a-Judge 的偏差怎么校准？](/notes/llm-judge-calibration)
