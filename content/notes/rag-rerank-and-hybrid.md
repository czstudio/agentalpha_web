...
excerpt: "向量检索负责把可能相关的证据捞进候选集，重排再把真能回答问题的片段顶到前面。一个管找得全，一个管排得准，不是二选一。"
series: "RAG"
seriesNo: "03"
number: "12"
minutes: 21
...
还有两个容易踩的坑：RRF 只看排名，不知道各路检索的质量是否同样可靠；它的输入也应该是经过租户和索引版本校验的候选 ID。若一条检索路径返回了旧版本文档，RRF 会很公平地把它排上来。公平不等于正确。
...
## 九、candidate-k、top-n 和 token 预算是三个旋钮
