...
excerpt: "Agent 降本先找成本结构，再改路由和上下文；盲目换小模型往往只把失败和重试变多。可靠的降本方案要同时守住任务成功率、延迟和安全边界。"
series: "项目深挖"
seriesNo: "11"
number: "53"
minutes: 22
...
但 Agent 的成本不只来自一次模型调用：上下文变长会增加输入 token，工具重试会增加请求次数，检索和 OCR 有自己的费用，人工接管和失败补偿也是真实成本。小模型如果更容易走错工具，最终账单可能更高。
...
按 `trace_id` 聚合，而不是只按模型账单看。一个“成功”任务如果经历了四次重试和一次人工接管，就不能按第一次调用的价格计算。
...
账单只能告诉你“花了多少钱”，不能告诉你“为什么花”。线上排查时，我会让每一笔成本都带着同一个 `trace_id`，并把模型、检索、工具、存储、人工和失败恢复拆成可聚合的事件：
