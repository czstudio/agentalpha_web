...
很多回答从 `Q、K、V` 开始，然后一路讲到 softmax，最后停在“这样就可以关注重要信息”。听起来没错，但面试官还会继续追问：
...
这一篇把这些问题串成一条可以复述、可以推导、也可以写出代码的链路。读完后，你不只知道公式长什么样，还能把一个 token 如何“找信息、拿信息、合并信息”讲清楚。
...
模型在处理“它”时，需要从前面的 token 中找到更可能对应的对象。最朴素的做法是把所有 token 平均相加，但平均会把无关信息也混进去；只看最近一个 token，又可能错过很远的指代。
