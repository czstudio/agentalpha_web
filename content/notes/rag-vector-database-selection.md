...
title: "RAG 选向量库不能只看排行榜：先把检索目标和边界算清楚"
excerpt: "向量库不是 RAG 的魔法核心。先定义语料规模、过滤条件、更新频率、召回目标和尾延迟，再在 HNSW、倒排、混合检索和托管服务之间做选择。"
series: "RAG"
seriesNo: "03"
number: "65"
minutes: 22
...
面试官问“你们为什么选这个向量数据库”，很多回答会变成产品名清单：支持向量、支持分片、支持云部署。真正有用的回答应该从任务反推：检索是追求语义召回，还是需要精确过滤？数据每天更新几次，还是每秒写入？能不能接受近似结果？最坏延迟和运维成本是多少？

## 先给一个能复述的答案

向量库的选择顺序应该是：先定召回与过滤目标，再定索引结构和存储形态，最后比较吞吐、P95、更新延迟、故障恢复和成本。小规模、强过滤、频繁更新的知识库可以先用关系库或搜索引擎的向量能力；规模变大、查询稳定后，再考虑 HNSW、IVF 或专用向量服务。不要为了“更像 AI”提前引入复杂索引。
