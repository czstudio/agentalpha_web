...
title: "工具调用失败后，Agent 应该重试、换工具还是停下来？"
excerpt: "失败不是一个布尔值。先判断请求有没有到达、外部副作用是否未知、参数是否有问题，再决定重试、换工具、回退还是转人工。"
series: "工具调用"
seriesNo: "10"
number: "45"
minutes: 24
...
一个排班 Agent 调用“改班接口”时等了 10 秒，客户端显示超时。值班同学准备重试，数据库却告诉他：第一次请求其实已经成功。第二次重试把同一个员工改到了另一个班次。

这类事故的根因不是“重试次数太多”，而是系统没有区分**没执行、执行失败、执行结果未知**。Agent 面对工具异常，首先要做的是判断状态，再谈补救。
...
一条工具调用至少经过客户端、网关、工具服务和外部系统。错误发生的位置不同，处理方式完全不同：
