...
excerpt: "一个 Agent 说自己成功了，不代表它真的完成了任务。把任务结果、步骤质量、工具调用、成本和安全约束分层，才能知道系统哪里可靠、哪里只是在碰运气。"
series: "评测"
seriesNo: "09"
number: "39"
minutes: 20
...
这不是一个“成功率还不够高”的问题，而是评测对象定义错了。Agent 不是一次性函数，它会规划、检索、调用工具、修改状态，再生成回复。只看最后一句话，就像只看快递是否送到，却不检查有没有未经授权开箱、重复扣款和超时绕路。
...
“帮我查一下订单”不是一个可评测目标。它至少要拆成：读取哪一位用户的订单、使用什么权限、返回哪些字段、是否允许改变状态、多久完成、找不到时怎么说。
