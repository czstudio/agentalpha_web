...
title: "MCP 解决了什么问题？工具协议标准化之后仍有哪些坑"
excerpt: "MCP 把模型与外部工具、资源和提示模板的连接方式标准化了，但它没有自动解决身份、权限、版本、租户隔离和副作用控制。协议统一只是起点。"
series: "工具调用"
seriesNo: "10"
number: "46"
minutes: 25
...
团队把十几个内部系统接进 Agent 后，产品经理问：“既然都有工具了，为什么还要 MCP？”工程师回答：“为了统一。”再追问统一了什么，现场就安静了。

MCP 的价值不是给每个工具换一个新名字，而是约定主机、客户端和服务端如何发现能力、交换结构化消息、读取资源和处理生命周期。它减少了集成胶水，但不会替你决定哪些工具能被谁调用，更不会让一个危险写操作自动变安全。
...
同一台 Host 可以连接多个 Server。风险也因此从“一个 API key”变成了多条信任链：哪个 Server 能读什么，返回的资源是否可信，工具调用是否经过审批，都要逐项回答。
