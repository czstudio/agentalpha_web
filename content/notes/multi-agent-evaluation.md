...
title: "如何评测多 Agent 协作？不能只看最终答案"
excerpt: "最终答案正确，不代表协作过程可靠。多 Agent 评测要把个体能力、消息质量、协作效率、失败恢复和安全边界拆开看。"
series: "多智能体"
seriesNo: "07"
number: "33"
minutes: 19
...
一个多 Agent 系统在测试集上 90% 的答案是对的，线上却经常出现三种问题：研究 Agent 找到了证据但没有传给写作 Agent；某个失败分支一直重试吃光预算；发布 Agent 在审核事件到达前就执行了动作。

如果只看最终答案，这些故障全部会被藏起来。多 Agent 的评测对象不是一张结果表，而是一条有角色、有消息、有状态、有成本和有风险的协作轨迹。
...
五层不是五个互不相关的仪表盘。任务成功率下降时，要能沿 trace 看到是哪个产物不合格、哪条消息被丢失、哪个重试放大了成本。

## 二、把评测集做成“任务 + 扰动”

正常任务只能验证 happy path。更实用的样本结构是：
