...
title: "如何给工具调用做权限控制和审计？"
excerpt: "Agent 的权限不是给一个 API key 就结束。把主体、租户、资源、动作、审批、幂等和审计证据放进同一条调用链，才能知道谁在什么条件下让系统做了什么。"
series: "工具调用"
seriesNo: "10"
number: "48"
minutes: 25
...
“让 Agent 自动处理退款”听起来像效率项目，真正上线时却变成了权限项目：它能看到哪些订单？谁允许它退款？用户改口后原来的授权还算不算？执行超时后重试会不会退两次？

如果这些问题只能靠 Prompt 里的“请谨慎操作”，那不是权限控制，只是把事故希望寄托在模型今天心情不错。
...
少一个维度，都可能出现“权限看起来正确，实际目标错了”。尤其是参数：用户批准退款 100 元，不代表 Agent 后来把金额改成 1000 元也有授权。
