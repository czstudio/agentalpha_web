...
title: "为什么一个 Agent 做不完的事，要拆成多个 Agent？"
excerpt: "多 Agent 的价值不在于把一个聊天窗口复制几份，而在于把互相牵制的目标拆成可验收的责任边界。先判断是否值得拆，再决定角色、交接和失败回退。"
series: "多智能体"
seriesNo: "07"
number: "29"
minutes: 18
...
一个“研究并发布行业报告”的 Agent，通常要同时浏览网页、筛选证据、做数据分析、写稿、审稿和发布。把这些能力都塞进一个大提示词里，Demo 可能能跑，线上却很难解释：它为什么跳过了引用？谁负责确认数字？发布前谁有权按下按钮？

多 Agent 不是把一个模型复制成五个窗口，而是把任务拆成几条可以独立验收的责任链。拆得好，专业能力能并行，风险动作有闸门；拆得坏，只会多出五份上下文、五次网络调用和一场互相甩锅。
...
只有在任务需要动态选择下一步、不同角色有不同判断标准，或者某一步的上下文与权限必须隔离时，多 Agent 才有明显价值。
