...
title: "Function Calling 不是让模型‘会调用函数’：参数、协议和验证"
excerpt: "Function Calling 真正解决的是把自然语言决策变成可校验的动作请求。Schema、解析、权限和执行回执缺一环，模型就可能把一个看似漂亮的 JSON 变成线上事故。"
series: "工具调用"
seriesNo: "10"
number: "44"
minutes: 24
...
“帮我把这笔订单退了。”客服 Agent 返回了一段 JSON，看起来字段齐全，业务却不敢执行：金额是字符串，订单号来自用户上一句里的一段引用，退款原因还是模型自己补的。这个时候，问题不是模型“不会 Function Calling”，而是系统把一段建议误当成了已经验证过的命令。
...
工具描述不能只写“调用退款接口”。至少要说明参数类型、枚举范围、必填项、互斥关系和副作用。描述写得越像广告，执行层越难验证；描述写得像合同，错误才会在离开模型前暴露。
