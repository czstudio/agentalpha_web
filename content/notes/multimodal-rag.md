---
slug: "multimodal-rag"
title: "多模态 RAG 怎么检索图片、表格和文字"
excerpt: "多模态 RAG 不是给文本 RAG 再加一列 image_url，而是把文字、表格、图片和坐标组织成统一证据对象，再用跨模态召回、元数据过滤和引用回溯回答问题。"
series: "多模态"
seriesNo: "08"
number: "37"
minutes: 20
...
设备维修 Agent 的知识库里有维修手册、接线图和参数表。用户问“这根蓝线接哪个端子”，纯文本检索只能找到“蓝线”，却找不到图里的端子编号；只做图片向量检索，又无法把参数表的额定电压一起带回来。

## 先给一个能复述的答案

多模态 RAG 的核心是先把不同模态转成可追溯的证据对象，而不是把图片当作附件。文本段落、表格、图片区域和图表数据各自保留原始内容、坐标、来源页和语义摘要，再建立文本索引、向量索引和关系/元数据索引。查询时先判断问题需要哪类证据，跨模态召回候选，再按来源、时间、权限和空间关系过滤，最后组装带引用的证据包交给模型。答案必须能回到原文、原图或表格单元格，检索不到时明确拒答。
