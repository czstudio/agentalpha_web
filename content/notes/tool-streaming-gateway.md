...
excerpt: "工具流式传输不只是把文字切成小段，而是把一次调用拆成可排序、可恢复的事件流。网关要管顺序、背压、断线续传、权限和最终提交，体验和可靠性才不会互相打架。"
series: "工具调用"
seriesNo: "10"
number: "68"
minutes: 21
...
Agent 调用工具时，用户关心的是“现在到哪一步了，下一步会不会改我的数据”。如果前端只能等完整响应，检索、审批、写入之间的几秒钟就只剩一个转圈。流式网关要送出的不是每个 token，而是带语义、能追踪的生命周期事件。
