...
excerpt: "Tool streaming 不只是把文字切成小段，而是把工具调用拆成可排序、可恢复的事件流。网关要处理顺序、背压、断线续传、权限和最终提交，体验和可靠性才能同时成立。"
series: "工具调用"
seriesNo: "10"
number: "68"
minutes: 21
...
Agent 调用工具时，用户真正关心的是“现在进行到哪了，下一步会不会改变我的数据”。如果前端只能等完整响应，检索、审批、写入之间的几秒钟就会变成一个无法解释的转圈。流式网关的任务不是把每个 token 都吐给前端，而是把有语义的生命周期事件安全地送出去。
